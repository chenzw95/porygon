...
class BuildCog:
    ...
    async def on_command_error(self, ctx, error):
        ...
        if isinstance(error, discord.ext.commands.CommandOnCooldown):
            if discord.utils.get(ctx.message.author.roles, name="GitHub Contributors") or discord.utils.get(ctx.message.author.roles, name="Moderators"):
                await ctx.send("A build was last requested {:.0f} seconds ago. Rebuild anyway? `y/n`".format(error.cooldown.per - error.retry_after))
                ...
                await ctx.send("This command is on cooldown (executed {:.0f} seconds ago). You will be able to use this command again in {:.0f} seconds.".format(error.cooldown.per - error.retry_after, error.retry_after))
